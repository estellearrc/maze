...
Maze read_maze(const string &file_name)
{
  ...
  ifstream ifst(file_name); // ouverture en lecture
  if (!ifst.is_open())
  {
    cout << "Erreur d’ouverture de " << file_name << endl;
  }
  else
  {
    ...
    {
      Cell cell(0, 0);
      ifst >> cell; //reads one cell
      nb_cells_read++;
      // cout << "nb_cells_read = " << nb_cells_read << endl;
      // cout << "pos_next_line = " << pos_next_line << endl;
      //insert cell into the map
      pair<int, int> coord = make_pair(cell.m_x, cell.m_y);

      if (nb_cells_read == pos_next_line) //if a new line is read
      {
        new_cell = cell;
        m_cells.insert(pair<pair<int, int>, Cell *>(coord, &cell));
        char n = ifst.get(); //number of neighbours for the cell of the new line
        // cout << "n = " << n << endl;
        cell.m_nb_neighb = atoi(&n);
        pos_next_line += cell.m_nb_neighb + 1; //updates position of the new line cell
        if (nb_lines == 1)
        {
          //first line = the cell is the entrance of the maze
          entrance = &cell;
        }
        else
        {
          if (nb_lines == 2)
          {
            //second line = the cell is the exit of the maze
            exit = &cell;
          }
        }
        nb_lines++;
      ...
      { //if cell is a neighbor of new_cell
        map<pair<int, int>, Cell *>::iterator it;
        it = m_cells.find(coord);
        if (it != m_cells.end()) //if the neighbor already exists
          new_cell.add_neighb(it->second);
        else //if the neighbor doesn't exist
        {
          m_cells.insert(pair<pair<int, int>, Cell *>(coord, &cell));
          new_cell.add_neighb(&cell);
        }
      }
    }
...
int main()
{
  ...
  // save_maze(maze, "backup.txt");
  Maze m_test = read_maze("backup.txt");
  display(m_test);
